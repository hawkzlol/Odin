...
class WaypointPackSelectorScreen(private val parent: Screen?) : Screen(Component.literal("Waypoint Pack Manager")) {
    ...
    override fun repositionElements() {
        ...
        footerLayout.addChild(Button.builder(Component.literal("Done")) { mc.gui.setScreen(parent) }.width(80).build())
    ...
    private fun showCreateDialog() {
        mc.gui.setScreen(TextPromptScreen("Create Pack").apply {
            setCallback { name ->
                if (name.isNotBlank()) {
                    loading = true
                ...
                mc.gui.setScreen(this@WaypointPackSelectorScreen)
            }
        })
    }

    private fun showRenameDialog(oldName: String) {
        mc.gui.setScreen(TextPromptScreen("Rename Pack").apply {
            setCallback { newName ->
    ...
    private fun importFromClipboard() {
        ...
        mc.gui.setScreen(TextPromptScreen("Import as New Pack").setCallback { name ->
            if (name.isNotBlank()) {
                loading = true
                OdinMod.scope.launch {
